...
PlayerbotMgr::PlayerbotMgr(Player* const master) : m_master(master)
{
    ...
    m_confMaxNumBots = sWorld.getConfig(CONFIG_UINT32_PLAYERBOT_MAX_NUM_BOTS);
	m_confDebugWhisper = sWorld.getConfig(CONFIG_BOOL_PLAYERBOT_DEBUG_WHISPER);
	m_confFollowDistance[0] = sWorld.getConfig(CONFIG_FLOAT_PLAYERBOT_FOLLOW_DISTANCE_MIN);
	m_confFollowDistance[1] = sWorld.getConfig(CONFIG_FLOAT_PLAYERBOT_FOLLOW_DISTANCE_MAX);
	m_confCollectCombat = sWorld.getConfig(CONFIG_BOOL_PLAYERBOT_COLLECT_COMBAT);
	m_confCollectQuest = sWorld.getConfig(CONFIG_BOOL_PLAYERBOT_COLLECT_QUEST);
...
void PlayerbotMgr::HandleMasterIncomingPacket(const WorldPacket& packet)
{
    switch (packet.GetOpcode())
    {
        ...
        {
            if (!sWorld.getConfig(CONFIG_BOOL_PLAYERBOT_SELL_GARBAGE))
                return;
...
bool ChatHandler::HandlePlayerbotCommand(char* args)
{
    ...
    if (resultchar)
    {
        ...
        int maxnum = sWorld.getConfig(CONFIG_UINT32_PLAYERBOT_MAX_NUM_BOTS);
        if (!(m_session->GetSecurity() > SEC_PLAYER))
    ...
    if (resultlvl)
    {
        ...
        int maxlvl = sWorld.getConfig(CONFIG_UINT32_PLAYERBOT_RESTRICT_BOT_LEVEL);
        if (!(m_session->GetSecurity() > SEC_PLAYER))
            if (charlvl > maxlvl)
            {
                PSendSysMessage("|cffff0000You cannot summon |cffffffff[%s]|cffff0000. Its level is too high. (Current Max: lvl |cffffffff%u)", fields[1].GetString(), maxlvl);
                SetSentErrorMessage(true);
                delete resultlvl;
                return false;
            }
